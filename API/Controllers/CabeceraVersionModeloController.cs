...
namespace API.Controllers
{
    public class CabeceraVersionModeloController : ApiController
    {
        ...
        public object cabeceraVersionModelo_consultar()
        {
            object _respuesta = new object();
            RespuestaHTTP _http = _objCatalogoRespuestasHTTP.consultar().Where(x => x.codigo == "500").FirstOrDefault();
            try
            {
                var _listaCabeceraVersion = _objCabeceraVersionModelo.ConsultarCabeceraVersionModeloTodasLasVersiones().ToList();
